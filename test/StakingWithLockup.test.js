...
var stakingWithLockup = artifacts.require('StakingWithLockup');
var Vault = artifacts.require('Vault');
var mySlice = artifacts.require('Token');
var myDai = artifacts.require('Token');
...
contract("StakingWithLockup", function (accounts) {
  ...
  describe("setups", function () {
    it("retrieve deployed contracts", async function () {
      ...
      daiContract = await mySlice.deployed();
    ...
    it("Should initialize", async function () {
      ...
      await sliceContract.transfer(vaultContract.address, web3.utils.toWei("10000"), { from: owner });
    });
  });
  ...
  describe("stake()", function () {
    beforeEach(async function () {
      await sliceContract.transfer(user1, web3.utils.toWei("100"), { from: owner });
      await sliceContract.transfer(user2, web3.utils.toWei("100"), { from: owner });
      await sliceContract.transfer(user3, web3.utils.toWei("100"), { from: owner });
    });

    it("Should stake for user1", async function () {
     await sliceContract.approve(stkLckpContract.address, web3.utils.toWei("1"), { from: user1 });
    ...
    it("Should stake for user2", async function () {
      await sliceContract.approve(stkLckpContract.address, web3.utils.toWei("10"), { from: user2 });
    ...
    it("Should stake lesser amount if exceeding reward cap", async function () {
      await sliceContract.approve(stkLckpContract.address, web3.utils.toWei("15"), { from: user2 });
      ...
      expect(result.amount).to.be.bignumber.equal(web3.utils.toWei("15"));
      expect(result.reward).to.be.bignumber.equal(web3.utils.toWei("4.5"));
    });

    it("Should not stake if amount entered is 0", async function () {
      await sliceContract.approve(stkLckpContract.address, web3.utils.toWei("10"), { from: user3 });
  ...
  describe("claim() and massClaim()", function () {
    ...
    beforeEach(async function () {
      ...
      await sliceContract.approve(stkLckpContract.address, web3.utils.toWei("100"), { from: user1 });
      await sliceContract.approve(stkLckpContract.address, web3.utils.toWei("100"), { from: user2 });
      await sliceContract.approve(stkLckpContract.address, web3.utils.toWei("100"), { from: user3 });
    });
    ...
    it("Should claim for user1", async function () {
      ...
      expect(reward).to.be.bignumber.equal(web3.utils.toWei("699.1"));
    ...
    it("Should claim for user2", async function () {
      ...
      expect(reward).to.be.bignumber.equal(web3.utils.toWei("777"));
    ...
    it("Should not claim if staking duration has not ended", async function () {
      ...
      const balance = await sliceContract.balanceOf(stkLckpContract.address);
      expect(balance).to.be.bignumber.equal(web3.utils.toWei("31"));

      const reward = await sliceContract.balanceOf(user3);
      expect(reward).to.be.bignumber.equal(web3.utils.toWei("895"));
    ...
    it("Should not be able to claim twice", async function () {
      ...
      const reward = await sliceContract.balanceOf(user3);
      expect(reward).to.be.bignumber.equal(web3.utils.toWei("996.5"));
    ...
    it("Should mass claim for user2", async function () {
      ...
      const reward = await sliceContract.balanceOf(user2);
      expect(reward).to.be.bignumber.equal(web3.utils.toWei("1083"));
      ...
      res = await sliceContract.balanceOf(vaultContract.address)
    ...
    it("Should not mass claim if any one of the stakes does not exist(already been claimed or never staked)", async function () {
      ...
      const balance = await sliceContract.balanceOf(stkLckpContract.address);
      expect(balance).to.be.bignumber.equal(web3.utils.toWei("51"));

      const reward = await sliceContract.balanceOf(user2);
      expect(reward).to.be.bignumber.equal(web3.utils.toWei("1164"));
    ...
    it("Should not mass claim for user2 if maturity period is not over for at least one stake", async function () {
      ...
      const balance = await sliceContract.balanceOf(stkLckpContract.address);
      expect(balance).to.be.bignumber.equal(web3.utils.toWei("81"));

      const reward = await sliceContract.balanceOf(user2);
      expect(reward).to.be.bignumber.equal(web3.utils.toWei("1234"));
  ...
  describe("repeal()", function () {
    ...
    it("Should not repeal if duration index is invalid", async function () {
      await expectRevert(stkLckpContract.stake(web3.utils.toWei("1"), 4, { from: user1 }), "StakingWithLockup: This duration has been repealed!");
      await expectRevert(stkLckpContract.stake(web3.utils.toWei("1"), 4, { from: user2 }), "StakingWithLockup: This duration has been repealed!");
